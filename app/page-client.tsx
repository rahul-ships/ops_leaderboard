  ...
  return (
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(100px,1fr))",gap:7,marginBottom:14}}>
        ...
        {bkMode!=="realtime"&&<MC icon={TrendingUp} label="Conv" value={lt.cr+"%"} color={convColor(lt.cr).c} small/>}
...
function DashboardClient({ initialData }: { initialData: { NM: any[], RW: any[] } }) {
  ...
  var allP = useMemo(function(){
    ...
    c.sort(function(a,b){
      var d=sortDir==="desc"?(b[effectiveSort]||0)-(a[effectiveSort]||0):(a[effectiveSort]||0)-(b[effectiveSort]||0);
      if(d!==0) return d;
      var d2=(b.bk||0)-(a.bk||0); if(d2!==0) return d2;
  ...
  var top3 = useMemo(function(){
    return allP.slice().sort(function(a,b){
      if (bkMode === "realtime") {
        var d=(b.bk||0)-(a.bk||0); if(d!==0) return d;
        return (a.name||"").localeCompare(b.name||"");
      }
      var d=(b.score||0)-(a.score||0); if(d!==0) return d;
      var d2=(b.bk||0)-(a.bk||0); if(d2!==0) return d2;
      return (a.name||"").localeCompare(b.name||"");
    }).slice(0,3);
  }, [allP, bkMode]);
        ...
        {view==="team"&&(
          <div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))",gap:8,marginBottom:14}}>
              ...
              {bkMode!=="realtime"&&<MC icon={TrendingUp} label="Avg Conv" value={(fd.length?(teamBk/fd.length*100).toFixed(1):"0")+"%"} color={C.pu}/>}
